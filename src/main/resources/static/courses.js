...
async function fetchCourses() {
  try {
    ...
    // Normalize database column names to match frontend usage
    courses = data.map(course => ({
      courseId: course.CourseID,      // Must be INT in database
...
function enrollCourse(courseId) {
  ...
  fetch("http://localhost:4000/api/enroll", {
    method: "POST",
    headers: {
...
// Load courses from the database when the page loads
